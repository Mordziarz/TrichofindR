...
genome_files <- list.files(path = main_dir, pattern = "\\.(fna|fasta)$", full.names = TRUE)
  ...
  message("🚀 Starting batch analysis. Genomes found: ", n_genomes)
  ...
  process_genome_full <- function(f_path) {
    ...
    tryCatch({
      ...
      for (l in imldts_loci) {
        ...
        if (file.exists(f_amp)) {
          ...
          if (length(seqs) > 0) {
            ...
            seq_rc <- as.character(reverseComplement(seq_dna))
            
            fwd_p <- get(paste0(l, "_F"))
            rev_p <- get(paste0(l, "_R"))
            
            fwd_at_start <- find_primer_smart(seq_dna, fwd_p, position = "start")
            rev_at_end <- find_primer_smart(seq_dna, rev_p, position = "end")
            
            if (!(fwd_at_start && rev_at_end)) {
              seq_rc_dna <- reverseComplement(seq_dna)
              if (find_primer_smart(seq_rc_dna, fwd_p, position = "start")) {
                seq_dna <- seq_rc_dna
              }
            }
            
            out_s <- DNAStringSet(seq_dna)
            names(out_s) <- g_name
            writeXStringSet(out_s, file.path(main_dir, paste0(l, "_results_all"), paste0(g_name, ".fasta")))
            
            found_sequences_oriented[[l]] <- as.character(seq_dna)
          }
        }
      }
      ...
      if (length(found_sequences_oriented) > 0) {
        available_loci <- imldts_loci[imldts_loci %in% names(found_sequences_oriented)]
      ...
      for (s in matek_stages) {
        ...
        if (file.exists(f_amp_m)) {
          ...
          res_m <- res_m[res_m$average_pident_weighted >= 95, ] 
      ...
      l_tab <- last_blast_tables[["LNS2"]]
      a_tab <- last_blast_tables[["ACL1"]]
  ...
  for(l in imldts_loci) {
    dir_l <- file.path(main_dir, paste0(l, "_results_all"))
    files <- list.files(dir_l, full.names = TRUE)
    if(length(files) > 0) {
  ...
  message("✨ Batch analysis complete. All results saved in main directory.")
  return(final_table)
}